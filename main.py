...
class BookSearchEngine:
    """A class to encapsulate the functionality of a desktop application designed
    for searching, viewing, adding, updating, and deleting book records using a
    GUI. It interfaces with an SQLite database to perform data operations, providing
    users with a visually appealing and functional platform for managing their
    book collection.
    """
    def __init__(self):
        """Initializes an instance of the BookSearchEngine class
        """
        # Establish a connection to the SQLite database located at './assets/books.db'
        self.conn = sqlite3.connect(database='./assets/books.db')
        # Create a cursor object from the database connection to execute SQL commands
        self.cur = self.conn.cursor()
        # Execute the SQL command to create the 'books' table if it does not already exist
    ...
    def reset(self):
        ...
        """
        # Execute the SQL command to delete all records in the 'books' table.
        ...
        # Iterate over each row in the DataFrame and insert it into the 'books' table.
        for row_id in range(len(books_df)):
            row = books_df.loc[row_id]
            self.cur.execute(sql_queries.INSERT_RECORD,(row['title'], row['author'],
                                                        row['rating'], row['isbn']))
    ...
    def view_all_records(self):
        ...
        # Clear the list box
        self.clear_list_box()
        # Execute the SQL command to query all book records available in the database
    ...
    def search_records(self):
        """Method to search the book records in the database based on the user
        input in the GUI entry fields.
        The search is dynamic, allowing for partial and case-insensitive matches.
        """
        ...
        # Initiate empty lists for SQL query conditions and corresponding values
        list_entry_values = []
        list_conditions = []

        # Append the SQL query condition and value for Title to corresponding lists ...
        # ...if the title entry field is not empty
        if title.strip() != '':
            list_conditions.append('LOWER(title) LIKE ?')
            list_entry_values.append(f'%{title}%')

        # Append the SQL query condition and value for Author to corresponding lists ...
        # ...if the author entry field is not empty
        if author.strip() != '':
            list_conditions.append('LOWER(author) LIKE ?')
            list_entry_values.append(f'%{author}%')

        # Append the SQL query condition and value for Rating to corresponding lists ...
        # ...if the rating entry field is not empty
        if rating.strip() != '':
            list_conditions.append('rating = ?')
            list_entry_values.append(rating)

        # Append the SQL query condition and value for ISBN to corresponding lists ...
        # ...if the isbn entry field is not empty
        if isbn.strip() != '':
            list_conditions.append('isbn LIKE ?')
            list_entry_values.append(f'%{isbn}%')

        # Check if there are any conditions set
        if len(list_entry_values) > 0:
            ...
            # Execute the SQL command to search the book records in the database...
    ...
    def add_record(self):
        """Method to add a new book record to the database using the data entered
        by the user in the GUI input fields.
        """
        # Collect the data from entry fields
        new_record = (self.title.get(), self.author.get(),
                      self.rating.get(), self.isbn.get())
        # Execute the SQL command to add a new record to the database using the data
        self.cur.execute(sql_queries.INSERT_RECORD,new_record)
        # Clear the list box
        self.clear_list_box()
        # Display confirmation message and details of the added record in ...
        # ...the list box
        self.list_box.insert(END, 'The following record has successfully been added:')
    ...
    def update_record(self):
        """Method to update the currently selected book record in the database
        with the new data entered by the user in the GUI input fields.
        """
        # Check if any item in the list box has been selected
        if self.selected_row:
            # Collect updated data from input fields
            updated_record = [self.title.get(), self.author.get(),
                              self.rating.get(), self.isbn.get()]
            # Execute the SQL command to update the selected record in the ...
            # ...database with the new data
            self.cur.execute(sql_queries.UPDATE_RECORD,tuple(updated_record+[self.selected_row[0]]))
            # Clear the list box
            self.list_box.delete(0, END)
            # Display confirmation message and details of the updated record in ...
            # ...the list box
            self.list_box.insert(END, 'The following record has been updated:')
            self.list_box.insert(END, tuple([self.selected_row[0]]+updated_record))
    ...
    def delete_record(self):
        """Method to delete the currently selected book record from the database
        and updates the GUI to reflect the change.
        """
        # Check if any item in the list box has been selected
        if self.selected_row:
            # Execute the SQL command to delete the selected record from the database
            self.cur.execute(sql_queries.DELETE_RECORD,(self.selected_row[0],))
            # Clear the list box to remove any existing data
            self.list_box.delete(0, END)
            # Inform the user which record was deleted by displaying the record ...
            # ...details in the list box
            self.list_box.insert(END, 'The following record has been deleted:')
    ...
    def clear(self):
        """Method to clear both the list box and all entry fields in the GUI.
        """
        # Clear all entry fields in the GUI
        self.clear_all_entries()
        # Clear all items in the list box and reset its state back to NORMAL
        self.clear_list_box()


    def get_selected_row(self, event):
        """Event method to handle the selection of a row in the list box component.
        When a row is selected, this method is triggered to update the entry
        fields with the data from the selected row to allow viewing or updating.
        """
        # Check if any item in the list box has been selected
        if len(self.list_box.curselection()) > 0:
            # Retrieve the index of the currently selected item in the list box
            selected_index = self.list_box.curselection()[0]
            # Retrieve the data tuple for the selected item through its index
            self.selected_row = self.list_box.get(selected_index)
            # Clear all entry values
            self.clear_all_entries()
            # Input data from the selected row into each corresponding entry field
    ...
    def clear_list_box(self):
        ...
        to NORMAL.
        """
        # Reset the state of list box to NORMAL to ensure the items can be selected
    ...
    def run(self):
        ...
        scrollbars, and buttons for user interaction, and configures all visual
        elements using the customtkinter library and binds functions to buttons
        ...
        frame_entries = customtkinter.CTkFrame(master=window, fg_color='#2B2D31')
        # Position the frame on the window
        frame_entries.pack(pady = 30)

        # Create a label for the title entry field
        title_label = customtkinter.CTkLabel(master=frame_entries,
                                             text='Title',
                                             font=('sans-serif', 14),
                                             text_color='#CCCCCC')
        ...
        self.title_entry = customtkinter.CTkEntry(master=frame_entries,
        ...
        author_label = customtkinter.CTkLabel(master=frame_entries,
                                              text='Author',
                                              font=('sans-serif', 14),
                                              text_color='#CCCCCC')
        # Position the label inside the frame and align it to the right
        ...
        self.author_entry = customtkinter.CTkEntry(master=frame_entries,
        ...
        rating_label = customtkinter.CTkLabel(master=frame_entries,
                                              text='Rating',
                                              font=('sans-serif', 14),
                                              text_color='#CCCCCC')
        # Position the label inside the frame and align it to the right
        ...
        self.rating_entry = customtkinter.CTkEntry(master=frame_entries,
        ...
        ISBN_label = customtkinter.CTkLabel(master=frame_entries,
                                            text='ISBN',
                                            font=('sans-serif', 14),
                                            text_color='#CCCCCC')
        # Position the label inside the frame and align it to the right
        ...
        self.ISBN_entry = customtkinter.CTkEntry(master=frame_entries,
        ...
        frame_data_mgmt = customtkinter.CTkFrame(master=window, fg_color='#2B2D31')
        ...
        self.list_box = Listbox(master=frame_data_mgmt,
                                bg='#313338',
                                width=64,
                                height=23,
                                bd=1,
        ...
        x_scrollbar = customtkinter.CTkScrollbar(master=frame_data_mgmt,
        ...
        y_scrollbar = customtkinter.CTkScrollbar(master=frame_data_mgmt,
        ...
        # Configure the list box to link to the horizontal and vertical scrollbars
        self.list_box.configure(xscrollcommand=x_scrollbar.set,
                               yscrollcommand=y_scrollbar.set)

        # Bind the list box selection change event to the get_selected_row method
        self.list_box.bind('<<ListboxSelect>>', self.get_selected_row)

        # Set up a button that displays all records in the list box
        view_all_button = customtkinter.CTkButton(master=frame_data_mgmt,
                                                  text='View All',
                                                  fg_color='#5865f2',
                                                  hover_color='#2133ee',
                                                  height=40,
        ...
        # Set up a button that searches for the records meeting attribute conditions
        search_button = customtkinter.CTkButton(master=frame_data_mgmt,
                                                text='Search',
                                                fg_color='#5865f2',
                                                hover_color='#2133ee',
                                                height=40,
        ...
        add_button = customtkinter.CTkButton(master=frame_data_mgmt,
                                             text='Add',
                                             fg_color='#5865f2',
                                             hover_color='#2133ee',
                                             height=40,
        ...
        update_button = customtkinter.CTkButton(master=frame_data_mgmt,
                                                text='Update',
                                                fg_color='#5865f2',
                                                hover_color='#2133ee',
                                                height=40,
        ...
        delete_button = customtkinter.CTkButton(master=frame_data_mgmt,
                                                text='Delete',
                                                fg_color='#5865f2',
                                                hover_color='#2133ee',
                                                height=40,
        ...
        # Set up a button that clears all the entry values and displayed data on GUI
        clear_button = customtkinter.CTkButton(master=frame_data_mgmt,
                                               text='Clear',
                                               fg_color='#5865f2',
                                               hover_color='#2133ee',
                                               height=40,
